...
OBJECTS = [
    ...
    },
    {
        "name": "宏利货币B",
        "code": "000700",
        "source": "eastmoney_money_fund",  # 货币基金：通过天天基金API获取每日万份收益，累加计算年度收益率
        "ticker": "000700",
        "type": "money_fund"
    },
    {
...
def get_money_fund_yearly_returns(fund_code, start_year, end_year):
    """
    获取货币基金各年度收益率（通过天天基金API累加每日万份收益）
    
    货币基金没有净值变动，收益通过每日万份收益累加计算：
      年度收益率 = Σ(每日万份收益) / 10000 × 100%
    
    返回格式: {"yearly": {year: rate}}
    """
    ...
    for year in range(start_year, end_year + 1):
        ...
        # 基金成立日特殊处理
        if fund_code == '000700' and year == 2014:
            start = '2014-08-06'
        ...
        for page in range(1, 30):
            try:
                ...
                print(f"  [货币基金API] {year}年第{page}页获取失败: {e}")
                break
        
        if all_records:
            total_wfsy = sum(float(x['DWJZ']) for x in all_records)
            rate = round(total_wfsy / 10000.0 * 100.0, 2)
            result["yearly"][year] = rate
            print(f"  [货币基金API] {year}年: {len(all_records)}天, 收益率={rate}%")
        else:
            print(f"  [货币基金API] {year}年: 无数据")
...
def fetch_all_data():
    ...
    for obj in OBJECTS:
        ...
        if source == "eastmoney_money_fund":
            # 货币基金：通过万份收益累加计算年度收益率
            mf_result = get_money_fund_yearly_returns(ticker, START_YEAR, END_YEAR)
            money_fund_data[code] = mf_result
            print(f"  ✅ 货币基金数据获取完成: {len(mf_result['yearly'])} 年")
    ...
    return raw_data, eastmoney_data, money_fund_data


def compute_returns(raw_data, eastmoney_data, money_fund_data):
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        for obj in OBJECTS:
            ...
            if source == "eastmoney_money_fund" and code in money_fund_data:
                # 货币基金：月度全部为null，仅有年度收益率
                monthly = [None] * 12
                if year > current_year:
                    yearly = None
                else:
                    yearly = money_fund_data[code]["yearly"].get(year, None)
            elif source == "eastmoney_api" and code in eastmoney_data:
                # 使用天天基金API的官方收益率
...
def main():
    ...
    # 获取原始数据
    raw_data, eastmoney_data, money_fund_data = fetch_all_data()
